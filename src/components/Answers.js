...
class Answers extends Component {
  ...
  async tipAnswer(q, index, tipAmount) {
    ...
    await bihu.methods.tipAnswer(q, index).send({ from: accounts[0], value: tipAmount }).then(setTimeout(function () { window.location.reload(true) }, 28000))
  }
  async createAnswer(q, content) {
    ...
    const networkData = Bihu.networks[networkId]
    const bihu = web3.eth.Contract(Bihu.abi, networkData.address)
    await bihu.methods.createAnswer(q, content).send({ from: accounts[0] }).then(setTimeout(function () { window.location.reload(true) }, 26400))
